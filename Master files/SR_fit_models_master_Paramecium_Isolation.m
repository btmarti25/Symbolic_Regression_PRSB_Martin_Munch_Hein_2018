...
%%
Save_gp_as= "ParameciumIsolation_test" %file name of saved gp structure
rng('shuffle')
gp=gpdefaults();
gp.state.count =0;

%load data; define independent (x) and dependent (y) variables
data=csvread('ParameciumIsolation.csv',1,0);
[y, validation_data ] =datasample(data,round(length(data)/3),'Replace',false);
%validation_data=[43,40,28,49,22,30,14,25,19,13,53,46,33,23,10,41,24,52,12];
gp.xtrain=data(:,2:3); 
gp.xtrain(validation_data,:)=[];
gp.ytrain=data(:,4);
gp.ytrain(validation_data,:)=[];
gp.xtest=data(validation_data,2:3); 
gp.ytest=data(validation_data,4); 
