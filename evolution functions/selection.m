...
if rand >= gp.selection.tournament.p_pareto
    ...
    tour_ind = ceil(rand(gp.selection.tournament.size,1)*numel(gp.models.modelset));
    ...
    if number_of_best > 1
        ...
        if gp.selection.tournament.lex_pressure
            ...
            %tcomps = gp.fitness.complexity(tour_ind(bestfitness_tour_ind));
             tcomps = cell2mat(gp.models.num_coefs(tour_ind(bestfitness_tour_ind)));
    ...
    tour_comp  = cell2mat(gp.models.num_coefs(tour_ind));
 ...
 if isempty(ID)%BM all NaNs are selected pick random model
       ID=ceil(rand * gp.runcontrol.pop_size);
 end
