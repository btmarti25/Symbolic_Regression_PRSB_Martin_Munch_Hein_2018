...
%gp.runcontrol.pop_size=numel(gp.models.modelset);
...
while buildCount < num2build;
    ...
    if eventType == 1
        ...
        %BM
        parent = cellstr(gp.pop{parentIndex});
...
%         end
        ...
        %BM cellstr
%         if ~ischar(gp.pop{parentIndex});
%             1
%         end
        dad = cellstr(gp.pop{parentIndex});
...
%         end
        %BM cellstr
        mum = cellstr(gp.pop{parentIndex});
...
%             else
          ...
          %  end
            ...
            if iscell(dad{1})%BM
                newPop{buildCount} = cell2mat(dad{1});%BM;
            else
               newPop{buildCount} = dad{1};%BM; 
            end %BM
            ...
            mum{1,mum_target_gene_num} = daughter;
                if iscell(mum{1})%BM
                    newPop{buildCount} = cell2mat(mum{1});%BM;
                else
                    newPop{buildCount} = mum{1};
                end
            end
...
for g=1:num2skim
    ...
    if g==1
        ...
        %[~,oldIndex] = sort(gp.fitness.complexity(bestInds)); % BM
       [~,oldIndex] = sort(cell2mat(gp.models.num_coefs(bestInds))); %
    ...
    if gp.runcontrol.usecache
     %   cachedData.complexity = gp.fitness.complexity(oldIndex,1); BM
